import * as mock from "/mockup/client-mock.js";
...
function labLog(message) {
  ...
  const el = document.getElementById("ingest-debug-log");
...
function initPanel() {
  ...
  window.addEventListener("librarain-mock-mode", function (event) {
    syncMockCheckbox();
    syncSubmitButton();
  });
  ...
  actions.addEventListener("click", function (event) {
    ...
    runScenario(scenario).catch(function (err) {
      ...
      if (labApi && labApi.labLogError) labApi.labLogError(msg);
      else labLog("Errore: " + msg);
